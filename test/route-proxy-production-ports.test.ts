...
test('API4F-05: 복원한 non-2xx receipt는 activated engine port의 unavailable 결과로 보존된다', async () => {
  ...
  assert.deepEqual(await adapter.getRouteReceipt({ id: 'from', lat: 35.1, lon: 129 }, { id: 'to', lat: 35.2, lon: 129.1 }, { maxNewProviderAttemptCount: 1 }), { result: 'unavailable', newProviderAttemptCount: 0, reused: false });
});
