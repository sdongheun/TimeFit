...
function confirmFixture(value = course(), overrides = {}, selectedSession = session) {
  ...
  return { screen, flow, calls, paramsUpdates, listeners, native: runtime.native, emitAppState: state => appStateListeners.forEach(listener => listener(state)), resume() { const projection = homeActiveCourseProjection(flow.activeVerifiedCourse, null, id => id); screen.unmount(); return runtime.mount(CourseConfirmScreen, { route: { params: projection.params }, navigation }); }, setRoute: v => { routeResult = v; }, resolve: v => resolveRoute(v) };
}
