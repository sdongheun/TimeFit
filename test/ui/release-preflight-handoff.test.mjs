...
test('QA release: failed handoff must restore an existing Live Activity update', async context => {
  ...
  const session = { nowIso: '2026-09-07T06:00:00Z', remainingMin: 120, arrivalBufferMin: 10, origin: { id: 'origin', label: 'O', lat: 35.1, lon: 129.1 }, destination: null };
  ...
  const flow = { activeVerifiedCourse: null, pendingNavigationAction: null, startActiveVerifiedCourse() { return this.activeVerifiedCourse = startActiveVerifiedCourse(session, course, () => 'active', undefined, () => 'run'); }, updateActiveVerifiedCourse() {}, clearActiveVerifiedCourse() {} };
  ...
  const screen = runtime.mount(CourseConfirmScreen, { route: { params: { session, course } }, navigation: { addListener() { return () => {}; } } });
  ...
  assert.deepEqual(local, before, 'failed external handoff must not retain a successful preparation revision');
});
