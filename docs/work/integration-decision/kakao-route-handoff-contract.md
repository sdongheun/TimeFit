...
[DEC-KAKAO-ROUTE-START-02](kakao-route-start-decision.md)로 코스 진행 기준만 교체했다. URL 조립·app→HTTPS→browser fallback·결과 enum은 보존한다. 코스는 클릭을 이동 의사로 선저장하고 정상 웹 종료에도 유지하며 외부 성공을 합성하지 않는다. 전체 실패 복구·재열기·학습 제외·주변 경계는 연결 문서를 따른다. 아래 ‘handoff 성공 때만 진행’ 조건은 철회 이력이다.
