...
`docs/work/`은 새 작업 세션이 읽는 **짧은 현재 작업 공간**이다. 역할별 기존 `작업기록.md`는 감사·결정 이력을 보존하는 append-only 기록이므로, 현재 작업에 직접 연결되지 않은 과거 구간을 처음부터 읽지 않는다.
...
완료된 작업은 역할별 historical record에 요약·수락 근거를 남긴다. 새 세션의 상세 명령은 같은 목표와 의존성을 공유하는 작업 묶음 파일 하나에만 쓴다. 작업마다 파일을 기계적으로 늘리지 않는다.
