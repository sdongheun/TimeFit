...
2. 출시 자동 추천은 실제 경로·구조화 운영시간·최소 20분·도착 여유를 통과한 **한 장소**만 계산한다. 첫 결과는 대표 `1개`와 서로 다른 검증된 1곳 대안 최대 `3개`를 목표로 하지만 검증 결과가 적으면 수를 부풀리지 않는다. 미검증 single 후보가 남으면 `다른 장소 더 보기`로 다음 실제 검증 결과를 최대 3개씩 기존 목록 뒤에 누적하며 화면 총량에는 별도 고정 상한을 두지 않는다. 2·3곳 조립과 candidate-to-candidate 경로는 계속 사용하지 않는다.
