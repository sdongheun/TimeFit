...
import { createSetupGpsInitialization, createSetupRunGuard, unifiedSetupLayout } from './timeSetup/unifiedSetupModel';
...
export function TimeSetupScreen({ navigation, route }: Props) {
  ...
  // 최초 진입의 허용된 GPS만 조회한다. 수동 선택/이탈은 좌표와 라벨 모두 무효화한다.
  useEffect(() => {
    if (page !== 'setup' || origin || searchTarget || mapTarget) return;
    const request = gpsInitialization.begin();
    if (request === null) return;
    let active = true;
    const current = () => active && gpsInitialization.isCurrent(request);
    void (async () => {
      try {
        const permission = await Location.getForegroundPermissionsAsync();
        if (!current() || permission.status !== 'granted') return;
      ...
      } catch { if (current()) setOriginLabel('현재 위치 또는 출발지 선택'); }
    })();
    return () => { active = false; };
  }, [page, locationLabel, gpsInitialization]);
  useEffect(() => {
    const invalidate = () => { gpsInitialization.invalidate(); pickerRequest.current++; locationEditingSession.end(); };
    const unsubscribe = navigation.addListener('blur', invalidate);
    return () => { invalidate(); unsubscribe(); };
  }, [navigation, gpsInitialization, locationEditingSession]);
  ...
  const applyPlace = (target: Exclude<PickTarget, null>, point: LatLon, label: string, source?: 'device' | 'provider' | 'map') => {
    ...
    setError('');
    if (target === 'origin') { gpsInitialization.invalidate(); setOrigin(point); setOriginLabel(label); }
  ...
  const startManualRecommendation = async (captchaToken?: string, proxyEnabled = routeProxyEnabled) => {
    ...
    const session: RecommendationSession = { nowIso, origin: { id: 'origin', ...origin, label: originLabel }, destination: target, remainingMin: sessionRemainingMin, arrivalBufferMin, ...(devicePoint ? { deviceLocationSnapshot: devicePoint } : {}) };
  ...
  return <View style={[s.root, { paddingTop: insets.top }]}>
    <ScrollView testID="setup-scroll" style={{ flex: 1 }} keyboardShouldPersistTaps="handled"
      ...
      <View pointerEvents={captchaVisible || searchTarget || mapTarget ? 'none' : 'auto'}>
        <UnifiedSetupInputs header={<Header title="자투리 시간 설정" compact />} clockLabel={testNowMin != null ? `테스트 시각 ${fmtHM(now.nowMin)} 기준` : `현재 시각 ${fmtHM(now.nowMin)} · 최대 2시간`}
  ...
  function pickers() {
    ...
    const pickerCenter = pickerInitialCenter(target, origin, devicePoint, SEOMYEON);
    ...
    return <>
      ...
      <PlacePicker visible={searchTarget !== null} title={target === 'origin' ? '출발지 선택' : '도착지 선택'} center={pickerCenter} showGps editingSession={locationEditingSession} labelAdapter={locationLabel}
