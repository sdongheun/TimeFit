...
type Page = 'setup' | 'route-setup' | 'time-picker' | 'test-clock' | 'qa-launcher' | 'loading';
...
export function TimeSetupScreen({ navigation, route }: Props) {
  ...
  const initialEnd = Math.min(23 * 60 + 59, now.nowMin + clampReleasePresetMinutes(route.params?.presetMin));
  ...
  const executeRecommendation = async (session: RecommendationSession, input: { captchaToken?: string; proxyEnabled: boolean; qaRunToken?: QaReleaseOneStopRunToken }) => {
    ...
    try {
      ...
      setLoadingStep(1);
      setLoadingStep(2);
      const result = await runRecommendationSession(session, { routeProxyEnabled: input.proxyEnabled, captchaToken: input.captchaToken });
      if (input.qaRunToken && !qaRunController.isCurrent(input.qaRunToken)) return;
      setLoadingStep(3);
      ...
      } else navigation.replace('Results', params);
    } catch (reason) {
      if (input.qaRunToken && !qaRunController.isCurrent(input.qaRunToken)) return;
  ...
  const startManualRecommendation = async (captchaToken?: string, proxyEnabled = routeProxyEnabled) => {
    ...
    if (sessionValidation) { setError(sessionValidation); return; }
  ...
  const run = () => {
    ...
    if (decision.kind === 'show_captcha') { setQaCaptchaRequest(null); setCaptchaVisible(true); return; }
    if (decision.kind === 'fail_missing_challenge') { setError('안전 확인을 준비하지 못했어요. 다시 시도해 주세요.'); return; }
    void startManualRecommendation(undefined, decision.kind === 'start_proxy');
  };
  ...
  const verifiedCaptcha = (captchaToken: string) => {
    ...
    setQaCaptchaRequest(null);
    if (!request) { void startManualRecommendation(captchaToken, true); return; }
  ...
  const Header = ({ title }: { title: string }) => <View style={s.header}><Pressable variant="icon" onPress={back} style={s.back}><Text style={s.backText}>‹</Text></Pressable><Text style={s.headerTitle}>{title}</Text><View style={s.back} /></View>;
  if (page === 'loading') return <View style={s.root}><View style={[s.loading, { paddingTop: insets.top + 40 }]}><Text style={s.loadingEyebrow}>{fmtHM(loadingUntilMin)}까지 계산 중</Text><Text style={s.loadingTitle}>시간 안에 들를 곳을{`\n`}찾고 있어요</Text>{['현재 위치와 도착지 확인', '이동 가능한 범위 계산', '짧게 들를 장소 찾기', '운영 상태 확인'].map((label, index) => <View key={label} style={s.loadingRow}><View style={[s.dot, index < loadingStep && s.dotDone, index === loadingStep && s.dotActive]} /><Text style={[s.loadingText, index <= loadingStep && s.loadingTextActive]}>{label}</Text></View>)}</View></View>;
  ...
  if (page === 'time-picker') return <View style={s.root}><ScrollView contentContainerStyle={[s.screen, { paddingTop: insets.top + 14 }]}><Header title="도착 시각" /><Text style={s.smallCopy}>현재 시각부터 최대 2시간 안에서 분 단위로 정합니다.</Text><View style={s.wheelPanel}><View style={s.meridiem}><TimeWheel accessibilityLabel="도착 시각 오전 오후" values={MERIDIEMS} index={pm ? 1 : 0} onChange={(index) => setPm(index === 1)} /></View><View style={s.wheelCol}><TimeWheel accessibilityLabel="도착 시각 시" values={HOURS_12} index={hour12 - 1} onChange={(index) => setHour12(index + 1)} /></View><View style={s.wheelCol}><TimeWheel accessibilityLabel="도착 시각 분" values={MINUTES} index={minute} onChange={setMinute} /></View></View>{validation && origin ? <Text style={s.error}>{validation}</Text> : null}<Pressable style={s.primary} onPress={() => setPage('setup')}><Text style={s.primaryText}>도착 시각 설정</Text></Pressable></ScrollView></View>;
