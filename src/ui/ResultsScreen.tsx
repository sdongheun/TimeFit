...
export function ResultsScreen({ route, navigation }: Props) {
  ...
  function confirmCourse() {
    ...
    navigation.navigate('Detail', { course, origin, ctx });
  }
    ...
    <View style={s.root}>
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.basket}>
          ...
          ) : <Text style={s.emptySmall}>마음에 드는 장소를 담아 코스를 만들어보세요.</Text>}
          ...
          <Pressable style={[s.cta, !selected.length && s.ctaOff]} disabled={!selected.length} onPress={confirmCourse}>
            <Text style={s.ctaTxt}>{selected.length ? '이 코스로 상세 보기' : '장소를 먼저 담아주세요'}</Text>
          </Pressable>
        </View>
