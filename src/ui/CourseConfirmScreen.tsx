...
function CourseConfirmContent({ route, navigation }: Props) {
  ...
  const [startExpired, setStartExpired] = useState(false);
  ...
  return <KeyboardAvoidingView style={s.root} behavior="padding"><View style={s.root}><ScrollView testID="course-confirm-scroll" keyboardShouldPersistTaps="handled" contentContainerStyle={[s.body, { paddingTop: insets.top + 14, paddingBottom: mode === 'active' && state && current ? Math.max(actionHeight, 96 + insets.bottom) + 16 : 34 }]}>{header}
    ...
    {mode === 'review' && startExpired ? <View><Text accessibilityRole="alert" style={s.error}>설정한 시간이 지났어요. 시간을 다시 설정해 주세요.</Text><Pressable testID="course-start-reset-time" style={s.primary} onPress={() => navigation.replace('TimeSetup')}><Text style={s.primaryText}>시간 다시 설정하기</Text></Pressable></View> : null}
...
import { canStartRecommendationSession, recommendationDeadlineLabel } from './timeSetup/datedSetupTime';
