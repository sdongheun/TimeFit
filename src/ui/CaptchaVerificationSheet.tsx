...
export function CaptchaVerificationSheet({ visible, challengeUrl, onVerified, onClose }: Props) {
  ...
  useEffect(() => {
    if (!visible) { firstFailure.current = null; setDiagnostic(null); setState('idle'); return; }
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={cancel}>
    <View style={s.scrim}><View style={[s.sheet, { paddingBottom: Math.max(insets.bottom, 20) }]} accessibilityViewIsModal>
      ...
      <Text style={s.copy}>추천을 시작하기 전에 안전 확인이 필요할 수 있어요.</Text>
      {state === 'loading' && validChallengeUrl ? <View testID="captcha-webview-wrap" style={s.webWrap}>
        <WebView
          ...
          onError={() => fail('webview_network_error')}
          onHttpError={() => fail('webview_http_error')}
          onMessage={(event) => {
            // A later bridge event must not turn a failed attempt into a verified request.
            if (firstFailure.current) return;
            ...
            if (message.type === 'token') { setState(captchaStateFor('token')); onVerified(message.token); return; }
            if (message.type === 'error') { fail('widget_error'); return; }
            setState(captchaStateFor(message.type));
          }}
        />
      </View> : null}
      {state === 'failed' ? <Text accessibilityRole="alert" testID="captcha-error" style={s.error}>안전 확인을 열지 못했어요. 다시 시도해 주세요.</Text> : null}
