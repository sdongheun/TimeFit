...
export function isAreaDependentSpot(
  spot: Pick<Spot, "category" | "subCategory" | "availabilityProfile">,
): boolean {
  if (spot.availabilityProfile === "area") return true;
  if (spot.category !== "상업지구" && spot.category !== "문화시설")
    return false;
...
export function areaAvailabilityDuring(
  spot: Pick<
    Spot,
    "contentId" | "title" | "category" | "subCategory" | "availabilityProfile"
  >,
  startMin: number,
  dwellMin: number,
